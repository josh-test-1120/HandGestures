...
#include <avr/wdt.h>
#include <Fonts/TomThumb.h> // 3x5 pixels
...
void setup() {
    ...
    //   display.println("OLED OK Josh Cool!");
    //   display.display();
    // }
    ...
    count = 1;
    delay(1000);
    //SPI.begin(); // Initialize SPI bus for SD card
    delay(500);  // Short delay
    // Initialize SD card
    if (!SD.begin(SD_CS_PIN)) {
        Serial.println("SD initialization failed!");
        return;  // Exit setup if SD card fails to initialize
    }
}

// Runs continually (final delay creates interval)
void loop() {
  //if (count == 49) initDataFile();
  ...
  // // --- DRAW TO OLED ---
  // display.clearDisplay();
  // //display.setFont(&TomThumb); // Use a small font
  // display.setTextSize(1);
  // display.setTextColor(SSD1306_WHITE);

  // // Top Line: Roll and Pitch
  // display.setCursor(0, 0);
  // display.print("AX:");
  ...
  // // // Middle: Progress Bar (for Roll)
  // int barCenter = SCREEN_WIDTH / 2;
  // int barLength = map((int)roll, -45, 45, -50, 50);  // Map -45 to 45 degrees to -50 to 50 pixels

  // display.drawLine(barCenter, 20, barCenter + barLength, 20, SSD1306_WHITE);
  // display.drawLine(barCenter-1, 19, barCenter-1+barLength, 19, SSD1306_WHITE); // thicker bar
  // display.drawLine(barCenter, 21, barCenter+barLength, 21, SSD1306_WHITE);

  // // Bottom: small text "Left" and "Right"
  // display.setCursor(0, 24);
  // display.print("Left");

  // display.setCursor(SCREEN_WIDTH - 30, 24);
  // display.print("Right");
